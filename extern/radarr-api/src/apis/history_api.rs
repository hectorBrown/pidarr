...
pub async fn api_v3_history_get(configuration: &configuration::Configuration, page: Option<i32>, page_size: Option<i32>, sort_key: Option<&str>, sort_direction: Option<models::SortDirection>, include_movie: Option<bool>, event_type: Option<Vec<i32>>, download_id: Option<&str>, movie_ids: Option<Vec<i32>>, languages: Option<Vec<i32>>, quality: Option<Vec<i32>>) -> Result<models::HistoryResourcePagingResource, Error<ApiV3HistoryGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_page = page;
...
pub async fn api_v3_history_movie_get(configuration: &configuration::Configuration, movie_id: Option<i32>, event_type: Option<models::MovieHistoryEventType>, include_movie: Option<bool>) -> Result<Vec<models::HistoryResource>, Error<ApiV3HistoryMovieGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ApiV3HistoryMovieGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

pub async fn api_v3_history_since_get(configuration: &configuration::Configuration, date: Option<String>, event_type: Option<models::MovieHistoryEventType>, include_movie: Option<bool>) -> Result<Vec<models::HistoryResource>, Error<ApiV3HistorySinceGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_date = date;
    let p_event_type = event_type;
    let p_include_movie = include_movie;
